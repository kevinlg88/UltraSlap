...
float3 IndirectLighting(float3 albedo, float3 specularColor, BDRFCommonData commonData)
{
	float3 N = commonData.N;
	float3 V = commonData.V;

	float3 diffuse = ShadeSH(float4(N,1)) * albedo; 
...
float3 CalculateLighting_PBR(float3 albedo, EffectsData effectsData)
{
	...
	float3 indirectLighting = IndirectLighting(albedo, specularColor, commonData);
