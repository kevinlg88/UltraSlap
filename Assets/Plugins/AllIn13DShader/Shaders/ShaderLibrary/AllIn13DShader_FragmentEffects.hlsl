...
float4 DepthColoring(float4 inputColor, EffectsData data)
{
	...
	//float4 depthColor = SAMPLE_TEX2D(global_DepthGradient, float2(data.normalizedDepth, 1.0));
	
	float v = data.camDistance / global_DepthZoneLength;
	v = saturate(pow(v, global_DepthGradientFallOff));
	float4 depthColor = SAMPLE_TEX2D(global_DepthGradient, float2(v, 1.0));
	
	res.rgb = lerp(res.rgb, depthColor.rgb, depthColor.a);
	return res;
}
#endif
